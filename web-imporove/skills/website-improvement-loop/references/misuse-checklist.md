...
| Misuse | Example | Fix |
|--------|---------|-----|
| Missing viewport meta | Page not scaling on mobile | Add `<meta name="viewport" content="width=device-width,initial-scale=1">` |
| Oversized images | 4000×3000px photo served as-is | Resize + `<img sizes>` + responsive srcset |
| Touch target too small | 20×20px close button | Minimum 48×48px per WCAG 2.5.5 |
| Fixed footer covers content | Sticky CTA bar overlaps last paragraph | Add `padding-bottom` equal to footer height |
| Video autoplay on cellular | `<video autoplay>` on 4G | `autoplay` only on `prefers-reduced-data: no-preference` |
| Keyboard overlap | Input hidden behind virtual keyboard | `visual-viewport` API or scroll-into-view |
| Horizontal scroll | Content wider than viewport due to fixed-width element | `max-width: 100%` on all child containers |
| Font size too small | `font-size: 12px` on mobile text | Minimum 16px to prevent iOS zoom |
| Tap/click ambiguity | Double-tap needed because first tap is :hover | Use `@media (hover: hover)` for hover effects |
| Disabled zoom | `<meta name="viewport" content="user-scalable=no">` | Remove `user-scalable=no` — violates WCAG 1.4.4 |
| Form auto-fill broken | Mobile auto-fill suggestions hidden by custom inputs | Use native `<input>` with proper `autocomplete` attribute |
