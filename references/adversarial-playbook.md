...
## Group 7 — Tablet Breakpoint (new)
| ID | Attack | Expected | Watch for |
|----|--------|----------|-----------|
| A41 | Open on tablet 768×1024, portrait | Responsive layout | Horizontal scroll, broken grid, font too small |
| A42 | Rotate from portrait to landscape mid-form | Layout adapts, form state preserved | Lost input, broken layout, overlapping elements |
| A43 | Tap targets on tablet (touch, 768×1024) | All buttons/links ≥48px | Tiny targets, overlapping touch areas |
| A44 | Split-screen multitask (tablet, 50% width) | App works at 384px width | Broken layout, hidden CTAs |
...
## Scoring: Breach Score
Breach Score = severity_rank × reproducibility × exploitability
  severity_rank: S0=4, S1=3, S2=2, S3=1
  reproducibility: reproduced=1.0, flaky=0.5
  exploitability: 1.0 (direct impact) / 0.7 (indirect) / 0.3 (informational)
Score range 0–4. Fix threshold: ≥2.0 → must fix before next cycle.
